...
import sas_pipe.shared.data.entity_data as asset_data
...
class Asset(entity.Entity):
    ...
    def get_rel(self):
        ...
        if os.path.exists(path):
            file_path = None
            if os.path.isfile(self.read_variant(task=self.task, variant=self.variant)):
    ...
    def task_path(self):
        ...
        print os.path.isfile(variant_data)
        if os.path.isfile(variant_data):
