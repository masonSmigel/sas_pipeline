...
class PurePath(object):
    ...
    @property
    def suffixes(self):
        """A items of the final component's suffixes, if any."""
        name = self.name
        if name.endswith('.'):
            return []
        name = name.lstrip('.')
...
class Path(PurePath):
    ...
    @classmethod
    def cwd(cls):
        """Return a new path pointing to the current working path
        (as returned by os.getcwd()).
        """
        return cls(os.getcwd())

    def iterdir(self):
        """Iterate over the files in this path.  Does not yield any
    ...
    def unlink(self):
        ...
        If the path is a path, use rmdir() instead.
        """
        self._accessor.unlink(self)

    def rmdir(self):
        """
        Remove this path.  The path must be empty.
        """
